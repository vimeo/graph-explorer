<script src="../assets/js/jquery-1.7.2.min.js"></script>
<script src="../graphitejs/jquery-ui.min.js"></script>
<script src="../graphitejs/jquery.graphite.js"></script>
<script src="../graphitejs/graphite_helpers.js"></script>
<script src="../graphitejs/rickshaw/vendor/d3.min.js"></script>
<script src="../graphitejs/rickshaw/vendor/d3.layout.min.js"></script>
<script src="../graphitejs/rickshaw/rickshaw.js"></script>
<script src="../graphitejs/flot/jquery.flot.js"></script>
<script src="../graphitejs/flot/jquery.flot.selection.js"></script>
<script src="../graphitejs/flot/jquery.flot.time.js"></script>
<script src="../graphitejs/flot/jquery.flot.stack.js"></script>
<script src="../graphitejs/jquery.flot.axislabels.js"></script>
<script src="../assets/js/crc32.js"></script>
<link type="text/css" rel="stylesheet" href="../graphitejs/rickshaw/rickshaw.css">
<link type="text/css" rel="stylesheet" href="../assets/css/graphitejs.css">
